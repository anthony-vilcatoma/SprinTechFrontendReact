...
import { showAllDirectRequestToOneTechnicalAlreadyInvoice } from '../../apis/Client/DirectRequest'
...
export default function HistoryTechnicalRequestPage() {
    ...
    useEffect(() => {
        ...
        getUserInformation(decodedToken.sub, accessToken)
            .then(dataUser => {
                showAllDirectRequestToOneTechnicalAlreadyInvoice(accessToken, dataUser.data.body.id, 3)
                ...
                <div className="w-6/12  flex flex-col gap-y-7 justify-between mx-auto py-5 bg-white shadow-personalized rounded-2xl px-4">
                    {
                        directRequest.map((e, index) => ((
                            <ProblemClientComponent typeModal={"Technical"} key={index} renderComponent={() => {
                            }} e={e} />
